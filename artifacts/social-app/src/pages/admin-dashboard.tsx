...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "users" | "kyc" | "wallet" | "moderation" | "tickets"
  >("overview");
      ...
      <Shell>
        <div className="max-w-md mx-auto my-16 text-center glass-panel p-8 rounded-3xl border border-destructive/30 space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            Esta sección es exclusiva para el <strong>Superadministrador</strong> de MenpoeSocial. Si crees que esto es un error, inicia sesión con una cuenta autorizada.
          </p>
  ...
  return (
    <Shell>
      <div className="max-w-6xl mx-auto w-full p-4 pb-24 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <p className="text-xs sm:text-sm text-muted-foreground">
                Control maestro de usuarios, verificaciones KYC, billetera, moderación y soporte técnico.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="glass-panel p-5 rounded-2xl border border-border/40 space-y-3">
                <h3 className="text-base font-bold flex items-center gap-2">
                  <ShieldCheck className="w-5 h-5 text-primary" /> Verificaciones Recientes
                </h3>
                {verifications.slice(0, 4).map((v) => (
                  <div key={v.id} className="flex items-center justify-between p-3 rounded-xl bg-white/5 text-xs">
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="font-bold truncate">{v.entityTitle || v.userName}</p>
                        <p className="text-muted-foreground text-[10px]">{v.documentType} · {v.documentNumber}</p>
                      </div>
                    </div>
                    <Badge
                      className={
              ...
              <div className="glass-panel p-5 rounded-2xl border border-border/40 space-y-3">
                <h3 className="text-base font-bold flex items-center gap-2">
                  <Coins className="w-5 h-5 text-amber-400" /> Solicitudes de Recarga
                </h3>
                {pendingTopups.length === 0 ? (
                  ...
                  pendingTopups.slice(0, 4).map((t: any) => (
                    <div key={t.id} className="flex items-center justify-between p-3 rounded-xl bg-white/5 text-xs">
                      <div>
                        <p className="font-bold text-amber-400">{t.priceLabel || `$ ${t.tokens?.toLocaleString()} COP`}</p>
                        <p className="text-muted-foreground text-[10px]">Usuario: {t.userId}</p>
                      </div>
                      <Button
                        size="sm"
                        className="rounded-xl h-7 text-xs bg-amber-500 hover:bg-amber-600 text-black font-bold"
          ...
          <div className="space-y-4">
            ...
            {filteredVerifs.length === 0 ? (
              <div className="glass-panel p-12 rounded-3xl text-center text-muted-foreground space-y-2">
                ...
                <p className="text-xs">Las solicitudes enviadas por usuarios o comunidades aparecerán aquí.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredVerifs.map((v) => (
                  ...
                  >
                    ...
                    <div className="p-3 bg-white/5 rounded-xl text-xs space-y-1 font-mono">
                      ...
                      {v.additionalInfo && (
                        <div className="pt-1 text-[11px] text-muted-foreground font-sans">
                          Nota: {v.additionalInfo}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex gap-2">
                      {v.frontImageUrl && (
                        <div
                          className="w-20 h-14 rounded-lg overflow-hidden border border-border/50 cursor-pointer relative group"
                      ...
                      {v.selfieImageUrl && (
                        <div
                          className="w-20 h-14 rounded-lg overflow-hidden border border-cyan-400/50 cursor-pointer relative group"
                          onClick={() => setSelectedVerif(v)}
                        >
                          <img src={v.selfieImageUrl} className="w-full h-full object-cover" alt="Selfie" />
                          <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center text-[10px] text-cyan-300 font-bold">
                            Selfie
                          </div>
                        </div>
                      )}
                    ...
                    <div className="pt-2 flex gap-2">
                      <Button
                        size="sm"
                        className="flex-1 rounded-xl text-xs neon-btn font-bold"
                        onClick={() => setSelectedVerif(v)}
                      >
                        <Eye className="w-3.5 h-3.5 mr-1" /> Inspeccionar Documentos
                      </Button>
                    </div>
                  </div>
                ))}
        ...
        {/* ─── TAB 4: RECARGAS Y BILLETERA ─────────────────────────────────── */}
        {activeTab === "wallet" && (
          <div className="space-y-4">
            <h3 className="text-lg font-bold flex items-center gap-2">
              <Coins className="w-5 h-5 text-amber-400" /> Solicitudes de Recarga de Monedas COP
            </h3>
            ...
            {pendingTopups.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {pendingTopups.map((t: any) => (
                  <div key={t.id} className="glass-panel p-5 rounded-2xl border border-amber-500/30 space-y-3">
                    <div className="flex items-center justify-between">
                      <Badge className="bg-amber-500/20 text-amber-400 font-mono text-sm px-2.5 py-1">
                        💰 {t.priceLabel || `$ ${t.tokens?.toLocaleString()} COP`}
                      </Badge>
                      <span className="text-xs text-muted-foreground font-mono">
                        {format(new Date(t.createdAt), "d MMM, h:mm a", { locale: es })}
                      </span>
                    </div>

                    <div className="text-xs space-y-1">
                      <p><strong>ID Usuario:</strong> {t.userId}</p>
                      <p><strong>Tokens/COP a Acreditar:</strong> {t.tokens?.toLocaleString()}</p>
        ...
        {/* Modal de Inspección y Aprobación KYC */}
        {selectedVerif && (
          <AppModal open onClose={() => setSelectedVerif(null)} className="w-full max-w-2xl">
            <div className="glass-panel p-6 rounded-3xl border border-primary/30 max-h-[90vh] overflow-y-auto space-y-5">
              ...
              {/* Visualizador de Fotos de Documentos */}
              <div className="space-y-3">
                <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground block">
                  Documentos Adjuntos en Alta Resolución
                </span>
              ...
              <div className="p-4 rounded-2xl bg-white/5 border border-border/40 space-y-3">
                <div className="flex gap-3">
                  <Button
                    className="flex-1 bg-green-600 hover:bg-green-700 text-white rounded-xl font-bold py-3"
                ...
                <div className="pt-2 border-t border-border/30 space-y-2">
                  <span className="text-xs font-semibold text-destructive">O Rechazar solicitud:</span>
                  <Input
                    value={rejectionReason}
                    onChange={(e) => setRejectionReason(e.target.value)}
                    placeholder="Motivo del rechazo (ej. Foto borrosa, documento vencido...)"
        ...
        {selectedTicket && (
          <AppModal open onClose={() => setSelectedTicket(null)} className="w-full max-w-lg">
            <div className="glass-panel p-6 rounded-3xl border border-primary/30 space-y-4">
              ...
              <div className="flex gap-2">
                <Button
                  className="flex-1 rounded-xl neon-btn text-xs font-bold"
                  onClick={async () => {
